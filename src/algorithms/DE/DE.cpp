...
int main_DE() {
    ...
    for (int n_run = 0; n_run < nroMaxExec; n_run++) {
        ...
        // Desalocar população dessa execução (libera cromossomos e vetores de indivíduos)
        for (int num_ind = 0; num_ind < tamPop; num_ind++) {
            if (popVelha.indiv[num_ind].cromossomo != nullptr) {
                delete[] popVelha.indiv[num_ind].cromossomo;
                popVelha.indiv[num_ind].cromossomo = nullptr;
            }
            if (popNova.indiv[num_ind].cromossomo != nullptr) {
                delete[] popNova.indiv[num_ind].cromossomo;
                popNova.indiv[num_ind].cromossomo = nullptr;
            }
        }
        if (popVelha.indiv != nullptr) {
            delete[] popVelha.indiv;
            popVelha.indiv = nullptr;
        }
        if (popNova.indiv != nullptr) {
            delete[] popNova.indiv;
            popNova.indiv = nullptr;
        }
        delete random2;
    }
...
void de(const int n_run) {
    ...
    // Data to be saved
    time_run[n_run] = static_cast<double>(clock() - time_start) / static_cast<double>(CLOCKS_PER_SEC);
    file_gen[n_run] = gen;
