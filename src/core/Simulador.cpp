...
//-----------------	simulador -----------------//
Simulador::Simulador(const double tamX, const double tamY, const int raio, const double posX, const double posY, const int ang)
{
	this->tamX = tamX;
	this->tamY = tamY;
	this->raio = raio;
...
int Simulador::sensor(const int dist, const int ang_param) const
{
	...
	if (const int distancia = dist + raio; posX + distancia * cos(anguloRad) >= tamX || posX + distancia * cos(anguloRad) <= 0 ||
	                                 posY + distancia * sin(anguloRad) >= tamY || posY + distancia * sin(anguloRad) <= 0)
		return 1;
	return 0;
} // sensor