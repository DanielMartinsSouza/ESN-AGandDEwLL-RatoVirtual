...
void estVIG2::save(double** Madj_s, const double* weightVert_s, const int ll_type, const int m_type,
                   const int c_type) const
{
    ...
    if ((eVIGm_edge_file = fopen(name_p, "w")) == nullptr)
    {
    ...
    if (LL_f == 1)
    {
        ...
        if ((eVIGm_vert_file = fopen(name_p, "w")) == nullptr)
        {
...
void estVIG2::wVert(const int a, const double w)
{
    ...
    {
        weightVert[a] = w;
        //  weight of the vertex (variable importance) is the maximum (for all calls of wVert(int a, double w))
        if (w > max_weightVert)
    ...
    if (n_vert > 0)
    {
        ...
        for (int var_index = 0; var_index < n_vert; var_index++)
        {
            ...
            for (int k = 0; k < max_k_aux; k++)
            {
                ...
                if (const int var_index_next = map_var2vert[j]; var_index_next > -1 && G_rec->isThereEdge(
                    var_index, var_index_next) == 0)
                {
